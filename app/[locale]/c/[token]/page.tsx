...
import { store, TENANT_ZERO_STAFF } from "@/lib/store";
...
export default async function AcceptPage({ params }: { params: Promise<{ locale: Locale; token: string }> }) {
  ...
  const deal = (await store.getDealForStaff(TENANT_ZERO_STAFF, rel.dealId)) ?? null;
  const org = await store.getOrg(rel.orgId);
