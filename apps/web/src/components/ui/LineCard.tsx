...
export function LineCard({ stage, line, bridge, onNext, onInfo, nextName, locked, meta, minLines = 4, nodeId }: LineCardProps) {
  ...
  return (
    <section className={styles.card} data-line-card data-node-id={nodeId} aria-label={`Script line, stage ${stage}`}>
      ...
      <div className={styles.lineBox} style={{ minHeight: `calc(${minLines} * var(--fs-display) * var(--lh-tight))` }}>
        {onNext ? (
