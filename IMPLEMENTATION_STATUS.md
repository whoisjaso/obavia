...
## Visual-critic fix round 2 (verified by the same gates — typecheck, lint, vitest, build, 88 e2e — all green)
- Train drill stage (R2-01): the choice tiles render directly under the line/prospect context; the "why now"
  paragraph and the mirror list left the stage for the ⓘ sheet (a `⇄ n mirrors` chip opens it); the branch
  drill's own question is a teal `?` card when no synthetic prospect line carries it; the drill body reserves
  `184px + safe-area` under the docked Check hero so nothing renders beneath it.
- Script (R2-02): Publish is a 64 px glass bar fixed flush to the tab bar (same chrome), the list keeps 88 px clear.
- In-call (R2-03/05/06/10/12): the Previous-line control left the chip row (it sits right of End, disabled at the
  first line); chips shrink instead of clipping (kit `Chip` labels ellipsize, `min-width: 0`); the phone strip is
  two snapping rows with a right-edge fade (3–4 words visible, meaning lines in the sheet); the purple THEIR REFS
  lane always shows when a reference exists — non-merged references as cards, word-carried references as `◆`
  index chips (no duplicate cards); merged word cards carry a `◆ REF` chip; End keeps `max(24px, safe+12px)`
  under it; the reference sheet is title + `not ~~word~~` chip + `◉ observed` chip + one imperative line, with
  prohibited inferences behind "Use it right".
- Hang-up glyph (R2-04): `phone-off` is the filled call handset rotated 135° (`ICON_TRANSFORMS`); the outcome
  "No answer" tile uses the same glyph.
- Call review (R2-07): no fixed action pair; transcript · corrections · ⓘ sit in the outcome row.
- Interview (R2-08): the cue reads `Pick up to N` / `Pick one` from the question's select mode in a fixed slot
  (`data-next-reason="blocked|clear"`), so the bar never reflows.
- Ids off the stage (R2-09): record rows show a section glyph + title (id in the accessible name), the record
  page title is the section's short name and the pager shows `n / 207`, own-line chips use the node's substage
  label, History cards drop the call-id tag, "Reviewed Call" replaces the truncated section name, the matching
  count rides the Source chip as a trailing number.
- Queue / next-up record sheet (R2-11): header (avatar · name · role · company · city) and one glyph row
  (`◑ local time`, `☏ (555) 010-0001`, line) — tz id and E.164 live in accessible names.
- Me (R2-13): no dash placeholders; day one shows "Recall · Entry line" as the next drill; empty rings draw a
  tick track (`Ring ticks`). Mock (R2-14): one icon tone for every scenario tile.
- Typography (R2-15): Inter (500/600/700/800) ships via `next/font/google` on `<html>` as `--font-sans`; the system
...
- `docs/screenshots/v2/fix-round2/*.png` (20 shots from a one-off Playwright pass, not kept as a spec): drill
  Branches idle/selected/ⓘ, record sheet, History, Sources section, source record, interview with a selection,
  call review, in-call reference sheet at 430×932 and 1440×900.
