...
export function AcceptForm({ locale, token, t }: { locale: Locale; token: string; t: Dictionary }) {
  ...
  if (err === "declined") return <p className="card" role="status" data-testid="invite-declined">{t.accept.declined}</p>;
  ...
  return (
    <form action={action} className="card stack" noValidate>
      ...
      <div className="field">
        <label htmlFor="contact">{t.accept.verify} <span className="help">{t.accept.verifyHelp}</span></label>
        <input id="contact" name="contact" required autoComplete="off" aria-invalid={err === "contact_mismatch"} />
      </div>
      <div className="row">
        ...
        <button className="btn" type="submit" name="decision" value="decline" disabled={pending} data-testid="decline">{t.accept.decline}</button>
      </div>
    </form>
  );
}
